  ...
  try {
    ...
    if (transactions?.length > 0) {
      ...
      const result = transactions.map(({ txid, vout, status }) => {
        const depositedAmount = vout.reduce((acc, { scriptpubkey_address, value }) =>
          scriptpubkey_address === btcAddress ? acc + value : acc, 0);
        return {
...
exports.btctransfer = async (req, res, next) => {
  try {
    ...
    if (!fromAddress || !fromPrivateKeyWIF || !toAddress || !amount) {
      return res.status(400).json({ status: false, message: "Missing required fields (fromAddress, fromPrivateKeyWIF, toAddress, amount)." });
    }
